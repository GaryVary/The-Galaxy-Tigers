...
public class GT_Drone_Hullmod_Warden_Antiarmor extends BaseHullMod {
	...
	public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
		...
		if (stats.getNumFighterBays().getBaseValue() == 1) {
			if (!stats.getVariant().getWings().isEmpty() && !stats.getVariant().getWings().contains("gt_warden_support_antiarmor_wing")) {
				stats.getVariant().getWings().set(0, "gt_warden_support_antiarmor_wing");
				return;
			}
			else if (stats.getVariant().getWings().isEmpty()) {
				stats.getVariant().getWings().add(0, "gt_warden_support_antiarmor_wing");
				return;
			}
		}
		else {
			if (!stats.getVariant().getWings().isEmpty() && !stats.getVariant().getWings().contains("gt_warden_support_antiarmor_wing")) {
				stats.getVariant().getWings().add((int)stats.getNumFighterBays().getBaseValue()-1,"gt_warden_support_antiarmor_wing");
				return;
			}
		}

		for (String awing : dawings) {
			if (awing.contains("gt_warden_support_antiarmor_wing")) {
	...
	public String getDescriptionParam(int index, HullSize hullSize) {
		if (index == 0) return "integrated Warden (GT) support drone";
		if (index == 1) return "shred the armor of a nearby hostile ship";
	    ...
	    if (ship != null) {
            for (String hullmod : ship.getVariant().getNonBuiltInHullmods()) {
                if ((hullmod.contains("galaxytigers_warden_support") ||
                        ...
                        !hullmod.contains("galaxytigers_warden_support_antiarmor")
                ) {
