...
public class GT_Drone_Hullmod_Warden_Polyvalent extends BaseHullMod {
	...
	public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
		...
		if (stats.getNumFighterBays().getBaseValue() == 1) {
			if (!stats.getVariant().getWings().isEmpty() && !stats.getVariant().getWings().contains("gt_warden_support_wing")) {
				stats.getVariant().getWings().set(0, "gt_warden_support_wing");
				return;
			}
			else if (stats.getVariant().getWings().isEmpty()) {
				stats.getVariant().getWings().add(0, "gt_warden_support_wing");
				return;
			}
		}
		else {
			if (!stats.getVariant().getWings().isEmpty() && !stats.getVariant().getWings().contains("gt_warden_support_wing")) {
				stats.getVariant().getWings().add((int)stats.getNumFighterBays().getBaseValue()-1,"gt_warden_support_wing");
				return;
			}
		}

		for (String awing : dawings) {
			if (awing.contains("gt_warden_support_wing")) {
		...
		if (ship != null) {
			for (String hullmod : ship.getVariant().getNonBuiltInHullmods()) {
				if ((hullmod.contains("galaxytigers_warden_support") ||
						...
						!hullmod.contains("galaxytigers_warden_support_hullmod")
				) {
