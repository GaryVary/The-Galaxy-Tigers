...
public class GT_Drone_Hullmod_Sentry_Pressure extends BaseHullMod {
	...
	public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
		...
		if (stats.getNumFighterBays().getBaseValue() == 1) {
			if (!stats.getVariant().getWings().isEmpty() && !stats.getVariant().getWings().contains("gt_sentry_support_wing")) {
				stats.getVariant().getWings().set(0, "gt_sentry_support_wing");
				return;
			}
			else if (stats.getVariant().getWings().isEmpty()) {
				stats.getVariant().getWings().add(0, "gt_sentry_support_wing");
				return;
			}
		}
		else {
			if (!stats.getVariant().getWings().isEmpty() && !stats.getVariant().getWings().contains("gt_sentry_support_wing")) {
				stats.getVariant().getWings().add((int)stats.getNumFighterBays().getBaseValue()-1,"gt_sentry_support_wing");
				return;
			}
		}

		for (String awing : dawings) {
			if (awing.contains("gt_sentry_support_wing")) {
	...
	public String getDescriptionParam(int index, HullSize hullSize) {
		...
		if (index == 1) return "put pressure on a target";
		if (index == 2) return "use its laser to sustain small pressure";
		if (index == 3) {
			switch(hullSize) {
				case FRIGATE:
					return "frigate";
		...
		if (ship != null) {
			for (String hullmod : ship.getVariant().getNonBuiltInHullmods()) {
				if ((hullmod.contains("galaxytigers_warden_support") ||
						...
						!hullmod.contains("galaxytigers_sentry_support_hullmod")
				) {
