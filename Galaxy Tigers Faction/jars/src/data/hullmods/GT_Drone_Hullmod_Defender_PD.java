...
public class GT_Drone_Hullmod_Defender_PD extends BaseHullMod {
	...
	public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
		...
		if (stats.getNumFighterBays().getBaseValue() == 1) {
			if (!stats.getVariant().getWings().isEmpty() && !stats.getVariant().getWings().contains("gt_defender_PD_wing")) {
				stats.getVariant().getWings().set(0, "gt_defender_PD_wing");
				return;
			}
			else if (stats.getVariant().getWings().isEmpty()) {
				stats.getVariant().getWings().add(0, "gt_defender_PD_wing");
				return;
			}
		}
		else {
			if (!stats.getVariant().getWings().isEmpty() && !stats.getVariant().getWings().contains("gt_defender_PD_wing")) {
				stats.getVariant().getWings().add((int)stats.getNumFighterBays().getBaseValue()-1,"gt_defender_PD_wing");
		...
		for (String awing : dawings) {
			if (awing.contains("gt_defender_PD_wing")) {
