...
public class GT_Automated_Machinery extends BaseHullMod {

	private static float refitboostmult = 0.9f;
	private static float speedboostmult = 1.08f;
	private static float agilityboostmult = 1.15f;
	private static final String AUTOMATED_MACHINERY_EFFECT_ID = "galaxytigers_automated_machinery_effect";

	@Override
	public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
		stats.getFighterRefitTimeMult().modifyMult(AUTOMATED_MACHINERY_EFFECT_ID, refitboostmult);

		if (stats.getVariant().getHullMods().contains("expanded_deck_crew")){
			MagicIncompatibleHullmods.removeHullmodWithWarning(stats.getVariant(), "expanded_deck_crew", "galaxytigers_automated_machinery");
		}
	}
	...
	public String getDescriptionParam(int index, HullSize hullSize) {
		if (index == 0) return "no crew can come onboard";
	...
	public static String getAutomatedMachineryEffectId() {
		return AUTOMATED_MACHINERY_EFFECT_ID;
	}
}
