...
public class GT_Drone_Hullmod_Defender_Strike extends BaseHullMod {
	...
	public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
		...
		if (stats.getNumFighterBays().getBaseValue() == 1) {
			if (!stats.getVariant().getWings().isEmpty() && !stats.getVariant().getWings().contains("gt_defender_support_strike_wing")) {
				stats.getVariant().getWings().set(0, "gt_defender_support_strike_wing");
				return;
			}
			else if (stats.getVariant().getWings().isEmpty()) {
				stats.getVariant().getWings().add(0, "gt_defender_support_strike_wing");
				return;
			}
		}
		else {
			if (!stats.getVariant().getWings().isEmpty() && !stats.getVariant().getWings().contains("gt_defender_support_strike_wing")) {
				stats.getVariant().getWings().add((int)stats.getNumFighterBays().getBaseValue()-1,"gt_defender_support_strike_wing");
		...
		for (String awing : dawings) {
			if (awing.contains("gt_defender_support_strike_wing")) {
	...
	public String getDescriptionParam(int index, HullSize hullSize) {
	    if (index == 0) return "integrated Defender (GT) support drone";
		...
		if (ship != null) {
			for (String hullmod : ship.getVariant().getNonBuiltInHullmods()) {
				if ((hullmod.contains("galaxytigers_warden_support") ||
						...
						!hullmod.contains("galaxytigers_defender_support_strike_hullmod")
				) {
