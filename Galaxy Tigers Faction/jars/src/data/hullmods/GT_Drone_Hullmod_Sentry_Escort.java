...
public class GT_Drone_Hullmod_Sentry_Escort extends BaseHullMod {
	...
	public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
		...
		if (stats.getNumFighterBays().getBaseValue() == 1) {
			if (!stats.getVariant().getWings().isEmpty() && !stats.getVariant().getWings().contains("gt_sentry_support_escort_wing")) {
				stats.getVariant().getWings().set(0, "gt_sentry_support_escort_wing");
				return;
			}
			else if (stats.getVariant().getWings().isEmpty()) {
				stats.getVariant().getWings().add(0, "gt_sentry_support_escort_wing");
				return;
			}
		}
		else {
			if (!stats.getVariant().getWings().isEmpty() && !stats.getVariant().getWings().contains("gt_sentry_support_escort_wing")) {
				stats.getVariant().getWings().add((int)stats.getNumFighterBays().getBaseValue()-1,"gt_sentry_support_escort_wing");
				return;
			}
		}

		for (String awing : dawings) {
			if (awing.contains("gt_sentry_support_escort_wing")) {
	...
	public void advanceInCombat(ShipAPI ship, float amount) {
		List<String> dawings = ship.getVariant().getWings();
		for (String awing : dawings) {
			if (awing.contains("gt_sentry_support_escort_wing")) {
				ship.getVariant().getWing(dawings.indexOf(awing)).setRefitTime((Float) sizetimer.get(ship.getHullSize()));
				/*if (stats.getVariant().getHullSize() == HullSize.CAPITAL_SHIP)
					stats.getVariant().getWing(dawings.indexOf(awing)).setNumFighters(2);*/
			}
		}
	...
	public String getDescriptionParam(int index, HullSize hullSize) {
	    if (index == 0) return "integrated Sentry (GT) support drone";
		if (index == 1) return "block incoming fighters and missiles";
		if (index == 2) return "harass a nearby hostile entity";
		if (index == 3) {
			switch(hullSize) {
				case FRIGATE:
					return "frigate";
	...
	public boolean isApplicableToShip(ShipAPI ship) {
		return ship != null && !ship.getVariant().hasHullMod(HullMods.PHASE_FIELD) &&
								...
								!ship.getVariant().hasHullMod("galaxytigers_picket_support_emp_hullmod") &&
	...
	public String getUnapplicableReason(ShipAPI ship) {
		if (ship != null) {
			for (String hullmod : ship.getVariant().getNonBuiltInHullmods()) {
				if ((hullmod.contains("galaxytigers_warden_support") ||
						...
						!hullmod.contains("galaxytigers_sentry_support_escort_hullmod")
				) {
		...
		//if (ship != null && ship.getVariant().hasHullMod("defenderdronepd")) return "Ship has a Defender (GT) escort";
